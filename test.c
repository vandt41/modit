...
SDL_AppResult SDL_AppInit(void **appstate, int argc, char *argv[]) {
  ...
  if (!SDL_CreateWindowAndRenderer("examples/demo/infinite-monkeys", 640, 480,
                                   0, &window, &renderer)) {
...
SDL_AppResult SDL_AppIterate(void *appstate) {
  ...
  if (lines) {
    ...
    elapsed /= 60;
    hours = (int)elapsed;
    SDL_asprintf(&caption, "Monkeys: %d - %dH:%dM:%dS", monkeys, hours, minutes,
                 seconds);
    if (caption) {
  ...
  rect.x = x;
  rect.y = y;
  rect.w = ((float)(progress - text) / (end - text)) *
           (cols * SDL_DEBUG_TEXT_FONT_CHARACTER_SIZE);
